...
public class ParkingLotVO {
    ...
    public boolean entry(VehicleInfoProperties.VehicleInfo info) {
        if (currentSlots < info.getSize()) {
            InputOutputHelper.printOutput("Apology , but we don't have enough space for your vehicle");
            return false;
        }
    ...
    public boolean exit(VehicleInfoProperties.VehicleInfo info, float numberOfHours) {
        if (statusMap.get(info.getType()).incoming - statusMap.get(info.getType()).outgoing <= 0) {
            InputOutputHelper.printOutput("Apology , but not possible to exit as there aren't enough vehicles of your type standing");
            return false;
        }
        currentSlots += info.getSize();
    ...
    public void report() {
        ...
        statusMap.entrySet().stream().forEach(
                t ->
                        InputOutputHelper.printOutput(String.format("%s Entered %d , Exited %d , currently parking %d",
                                t.getValue().type, t.getValue().incoming, t.getValue().outgoing,
                                t.getValue().incoming - t.getValue().outgoing))

        );

        InputOutputHelper.printOutput(String.format("Space Available %d ", currentSlots));

        InputOutputHelper.printOutput(String.format("Total earnings %4.2f ", current_cost));
